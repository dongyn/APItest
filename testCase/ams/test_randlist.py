...
class test_randlist(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.url = baseurl + "/cms/v1.0/stream/randlist"

    # 将解密后的字符串转为字典
    def decrypt_to_dict(self, text, split_num, str_split):
        # print(str(aes.decrypt(text, 'r'))[2:])
        decrypt = str(aes.decrypt(text, 'r'))[split_num:].split(str_split)
    ...
    def test_01_getlivelist(self):
        ...
        if (response.status_code == 200):
            ...
            response_data = self.decrypt_to_dict(r_data, 3, '"provider_play_urls":null}')
